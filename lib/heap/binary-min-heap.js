...
const _checkIfSiftNeeded_ = Symbol('checkIfSiftNeeded');
...
const BinaryMinHeap = class BinaryMinHeap {
  ...
  /**
   * @name pop
   * @desc Remove the lowest (min) value from the heap and return it
   * @alias popKey
   * @return {Number}
   */
  pop () {
  ...
  [_siftUp_] (index) {
    ...
    if (this[_entries_][index][0] < parentKey) { // Min goes on top
      ...
      if (leftKey !== undefined && rightKey !== undefined) {
        if (leftKey < rightKey) { // Min goes on top
  ...
  [_siftDown_] (parentIndex) {
    ...
    let rightKey = this[_entries_][rightIndex] && this[_entries_][rightIndex][0];
    // If heap is unordered
    if (leftKey < parentKey || rightKey < parentKey) { // Min goes on top
      if (leftKey !== undefined && rightKey !== undefined) {
        if (leftKey < rightKey) { // Min goes on top
