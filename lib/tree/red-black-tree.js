...
class RedBlackTreeNode {
  ...
  remove (value, direction) {
    if (value < this.value && this.left.value !== null)
      return this.left.remove(value, 'left');
    if (value > this.value && this.right.value !== null)
      return this.right.remove(value, 'right');
    if (value !== this.value)
      return false;
    /**
     * Value Found
    ...
    // Re-arrange the tree in order to make it a valid red-black tree
    let child = this.isLeaf(this.right) ? this.left : this.right;
    console.log('---- #1');
    root.print();
    let newRoot = this.swapPositionWithNode(child);
    console.log('---- #2');
    root.print();
    ...
    if (this.isBlack()) {
      if (child.isRed()) {
        ...
        // TODO: Handle newRoot being set
        let _newRoot = child.removeCase1();
        if (_newRoot)  {
          console.log('newRoot #2');
          newRoot = _newRoot;
        }
      }
    }
    // Remove value from the tree using binary tree logic
    this.removeNode(this.value, this.parent, this.getParentDirection());
    return newRoot;
  }

  removeNode(value, parent, direction) {
      ...
      if (
        ...
        if (parent) {
          if (parent.value === null) {
            ...
            parent[direction] = new RedBlackTreeNode(null, 'black', parent);
          }
          return true;
        }
      ...
      let leftDepth = (this.left.value === null) ? 0 : this.left.getMaxDepth();
      let rightDepth = (this.right.value === null) ? 0 : this.right.getMaxDepth();
      ...
      let values = this[biggerSide].getValues('forEach');
      let newNodeValue = RedBlackTree.prototype['_' + minOrMax].call(this, ...values);
   ...
   * @return {RedBlackTreeNode}
   */
  swapPositionWithNode(node) {
    let swap = (node1, node2, parent1, parent2) => {
      if (node1 !== null && parent1 !== null) {
        parent1[node1.getParentDirection()] = node2;
      } else {
        if (node1 === null && node2.value !== null) {
          parent1.value = node2.value;
          parent1.color = node2.color;
          console.log('LOST', node2.value);
        }
      }
      if (node2 !== null && parent2 !== null) {
        parent2[node2.getParentDirection()] = node1;
      } else {
        if (node2 === null && node1.value !== null) {
          parent2.value = node1.value;
          parent2.color = node1.color;
          console.log('LOST', node1.value);
        }
      }
      if (node1 !== null) node1.parent = parent2;
      if (node2 !== null) node2.parent = parent1;
      // Return newRoot
      if (parent1 === null) return node2;
      if (parent2 === null) return node1;
      return;
    };
    console.log('# 1');
    console.log(this.value, this.parent && this.parent.value, this.left && this.left.value, this.right && this.right.value);
    console.log(node.value, node.parent && node.parent.value, node.left && node.left.value, node.right && node.right.value);
    swap(this.left, node.left, this, node);
    console.log('# 2');
    console.log(this.value, this.parent && this.parent.value, this.left && this.left.value, this.right && this.right.value);
    console.log(node.value, node.parent && node.parent.value, node.left && node.left.value, node.right && node.right.value);
    swap(this.right, node.right, this, node);
    console.log('# 3');
    console.log(this.value, this.parent && this.parent.value, this.left && this.left.value, this.right && this.right.value);
    console.log(node.value, node.parent && node.parent.value, node.left && node.left.value, node.right && node.right.value);
    let newRoot = swap(this, node, this.parent, node.parent);
    console.log('# 4');
    console.log(this.value, this.parent && this.parent.value, this.left && this.left.value, this.right && this.right.value);
    console.log(node.value, node.parent && node.parent.value, node.left && node.left.value, node.right && node.right.value);
    return newRoot;
...
export default class RedBlackTree {
  ...
  remove (value) {
    ...
    let newRoot = this.root.remove(value);
    if (newRoot) {
      this.root = newRoot;
    }
    return;
