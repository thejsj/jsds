 ...
 * 4. Add parent (why are parents needed?)
 * 5. Add minimum
 * 6. Add max
 * 7. Add successor
 * 8. Add predecessor
 */
const binarySearchTreeConstructor = function (TYPE) {
  return class BinarySearchTree {
    ...
    _insert (value) {
      ...
      if (value < this.value) {
        if (this.left instanceof this.constructor) return this.left._insert(value)
        this.left = new this.constructor(value, this.opts)
        return true
      }
      if (value > this.value) {
        if (this.right instanceof this.constructor) return this.right._insert(value)
    ...
    _remove (value, parent, direction) {
      ...
      if (value === this.value)
        return this._removeNode(value, parent, direction)
     ...
     * @param {String}
     */
    _removeNode (value, parent, direction) {
      ...
      let newNodeValue = this['_' + minOrMax](...this[biggerSide].getValues())
    ...
    _contains (value) {
      ...
      if (value < this.value && this.left instanceof this.constructor) return this.left._contains(value)
      if (value > this.value && this.right instanceof this.constructor) return this.right._contains(value)
      return false
    }
     ...
     * @private
     */
    _forEachDepthFirst (callback, level = 0) {
      let getValue = (x) => (x === null) ? null : x.value
      callback(this.value, level, getValue(this.left), getValue(this.right))
...
export default binarySearchTreeConstructor
